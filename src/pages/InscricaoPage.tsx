...
export default function InscricaoPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!events) throw new Error('Nenhum evento aberto encontrado.')
    ...
    return (
      <div className="max-w-lg mx-auto px-4 py-12 text-center space-y-4">
        ...
        <p className="text-gray-600 text-sm">
          A inscrição do clube <strong>{clubName}</strong> foi recebida e aguarda validação pela organização.
        </p>
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="space-y-4">
          ...
          {athletes.map((athlete, idx) => (
            <div key={idx} className="card space-y-3 relative border-l-4 border-efa-gold">
              <div className="flex items-center justify-between">
              ...
              <div className="grid sm:grid-cols-2 gap-3">
                ...
                <div>
                  ...
                  <input
                    type="number"
                    className="input"
                    placeholder={`Ex: ${CURRENT_YEAR - 8}`}
                    value={athlete.birthYear}
                    min={1990}
                    max={CURRENT_YEAR}
                    onChange={(e) => updateAthlete(idx, { birthYear: e.target.value })}
                    required
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => updateAthlete(idx, { category: e.target.value as Category })}
                >
              ...
              <div className="flex flex-col gap-2">
                <label className="flex items-center gap-2 text-sm cursor-pointer select-none">
                  <input
                    type="checkbox"
                    className="w-4 h-4 accent-efa-blue"
                    checked={athlete.isFederated}
                    onChange={(e) => updateAthlete(idx, { isFederated: e.target.checked })}
                  />
                  <span>
                    Federado na <strong>Federação Portuguesa de Esgrima (FPE)</strong>
                ...
                <label className="flex items-center gap-2 text-sm cursor-pointer select-none">
                  <input
                    type="checkbox"
                    className="w-4 h-4 accent-efa-blue"
                    checked={athlete.hasInsurance}
                    onChange={(e) => updateAthlete(idx, { hasInsurance: e.target.checked })}
                  />
                  <span>
                    Possui <strong>seguro desportivo válido</strong>
                    <span className="text-red-500 ml-1">*</span>
                  </span>
                </label>
              </div>
              ...
              <div className="flex justify-end">
                <span className="badge bg-efa-gold/10 text-efa-gold border border-efa-gold/30 font-semibold">
                  {CATEGORY_PRICES[athlete.category]}€
                </span>
              </div>
            </div>
          ))}
